...
JSON:"""
    try:
        resp = ai.chat.completions.create(model="mistral-small-latest", messages=[{"role":"user","content":prompt}], temperature=0.1, max_tokens=200)
...
# ---------- JOB SOURCES (abbreviated, but all present) ----------
...
def location_match(job, desired_location):
    ...
    return desired_location.lower().strip() in loc_norm
...
def agentic_job_search(title, location, num_per_source=8):
    all_jobs = []
    all_jobs.extend(search_serpapi(f"{title} {location}", location, num_per_source))
    all_jobs.extend(search_adzuna(title, location, num_per_source))
    all_jobs.extend(search_remotive(title, num_per_source))
    all_jobs.extend(search_remoteok(title, num_per_source))
    all_jobs.extend(search_findwork(f"{title} {location}", num_per_source))
    all_jobs.extend(search_jsearch(f"{title} {location}", location, num_per_source))
    ...
    combined = exact + others[:max(0, 50 - len(exact))]
    logging.info(f"Agentic search: {len(exact)} exact, {len(combined)} total.")
    ...
    while True:
        try:
            ...
            for msg in resp.json().get("items", []):
                try:
                    user = pb("GET", f"/collections/users/records/{msg['user']}").json()
                    profile = f"skills={user.get('skills','')}, desired job={user.get('desired_job_title','')}" if user else ""
                    prompt = f"You are a helpful career coach. {profile}\nUser: {msg['message']}\nRespond helpfully."
                    ai_resp = ai.chat.completions.create(model="mistral-small-latest", messages=[{"role":"user","content":prompt}], temperature=0.7, max_tokens=300)
                    pb("PATCH", f"/collections/chat_messages/records/{msg['id']}", json_data={"response": ai_resp.choices[0].message.content.strip()})
                    logging.info(f"Replied to chat {msg['id']}")
                except Exception as e:
...
def scraping_loop():
    while True:
        ...
        for user in users:
            ...
            jobs, _ = agentic_job_search(query, loc)
